...
test.describe('Dashboard', () => {
  ...
  test.skip('dashboard loads with stats row (3 stat cards)', async ({ page }) => {
  ...
  test.skip('recent activity list is visible', async ({ page }) => {
    await page.goto('/dashboard')
    const activityList = page.locator('[data-testid="recent-activity"], [class*="activity"], [class*="recent"]').first()
    await expect(activityList).toBeVisible()
  })

  test.skip('+ New Presentation button navigates to /create', async ({ page }) => {
    await page.goto('/dashboard')
  ...
  test.skip('navigation header shows all links', async ({ page }) => {
    await page.goto('/dashboard')
    await expect(page.locator('nav, header').first()).toBeVisible()
  })

  test.skip('user avatar dropdown opens with settings/signout', async ({ page }) => {
    await page.goto('/dashboard')
