...
import { STYLE_STEERS, ALL_RULE_IDS, buildLogoPrompt } from '../app/_lib/logo-engine/index.ts'
...
if (!experiment || !engine) {
  console.error('usage: run.mjs <steer|ablation|symbol|marktype> [--engine=gemini|gptImage] [--n=3]')
  process.exit(2)
}
...
await pool(cells, 6, async (cell) => {
  ...
  const prompt = buildLogoPrompt(brand, opts)
