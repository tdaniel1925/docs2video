...
The renderer runs on **AWS ECS Fargate** behind a load balancer. The app
reaches it through `VIDEO_ASSEMBLY_URL`.

> **This folder was called `vps/` until 2026-09-20.** There is no VPS and has
> not been for some time, but the name kept the wrong word in front of
> everyone who opened the repo — including in code comments, in the admin
> screens, and in the error advice shown when a render fails.
>
> The old version of this document described a Hetzner box at
> `root@5.161.215.156`, with `ssh` and `scp` commands you could copy and
> paste. That machine is gone; every one of those commands would fail now, or
> worse, reach whoever holds that address next. `build-context.sh` had said so
> in its own header for a while — "the box is gone, and with it the only place
> the image could actually be built" — but the one document a person actually
> opens was never updated to match.
...
| Task family | `docs2video-service` (`render-service/ecs-task-definition.json`) |
...
**Never hand-edit the running server.** Edit `render-service/server.js` in this repo and
...
```sh
bash render-service/build-context.sh
```

This exists because the Dockerfile does `COPY remotion /app/remotion` and there
is no `render-service/remotion` in the repo — the old script copied it onto the server at
...
`render-service/buildspec.yml` **verifies the push by digest** rather than trusting the
...
- **`render-service/redeploy.sh`** — written for the Hetzner box. It clones to
