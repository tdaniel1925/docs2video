...
// render-service/logo-enhance-deploy/Dockerfile.additions). The app calls this only when
...
// admin flag video_engine_v3 is ON. See render-service/render-v3-deploy/ for the full
...
// upload. Vercel just triggers this. Reuses render-service/commercial.js (ports
...
// (render-service/slides.js) → render DirectedVideo → upload. Vercel just triggers this.
...
app.post('/generate', authCheck, async (req, res) => {
  ...
  try {
    ...
    reportError({ source: 'render-service/generate', videoId, userId, stage: 'pipeline', message: err.message, detail: err.stack })
    try {
      await supabase.from('videos').update({
        status: 'failed',
