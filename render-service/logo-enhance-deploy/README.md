...
| File | Purpose |
|------|---------|
| `process-logo-endpoint.js` | The `/process-logo` Express handler to paste into `render-service/server.js` |
...
1. **Endpoint:** paste the marked block from `process-logo-endpoint.js` into
   `render-service/server.js`, after `authCheck` is defined (e.g. near `/extract-document`).
2. **Image:** add `Dockerfile.additions` into the VPS Dockerfile
   (`/root/video-service/Dockerfile`), after the apt block, before `CMD`.
3. **Deploy:** `bash render-service/logo-enhance-deploy/deploy.sh`
