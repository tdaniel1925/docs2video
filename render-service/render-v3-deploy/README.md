...
| File | Purpose |
|------|---------|
| `render-v3-endpoint.js` | The `/render-v3` Express handler to paste into `render-service/server.js` |
...
1. **Endpoint:** paste the marked block from `render-v3-endpoint.js` into
   `render-service/server.js` after `authCheck` (already done in the repo copy — verify with
   `grep -c render-v3 render-service/server.js`).
2. **Dockerfile:** ensure `render-service/Dockerfile` contains the `Dockerfile.additions`
   block (Chrome libs + `COPY remotion /app/remotion` + install). Order: after
   the apt/rembg blocks, before `COPY server.js`.
3. **Deploy:** `bash render-service/render-v3-deploy/deploy.sh`
