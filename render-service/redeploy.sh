...
# The video service runs on AWS ECS Fargate now — see render-service/DEPLOY.md.
#
...
cp "$TMP/render-service/redeploy.sh" "$DIR/redeploy.sh" 2>/dev/null || true
cp "$TMP/render-service/server.js" "$DIR/server.js"
cp "$TMP/render-service/slides.js" "$DIR/slides.js"
...
cp "$TMP/render-service/commercial.js" "$DIR/commercial.js"
...
cp "$TMP/render-service/Dockerfile" "$DIR/Dockerfile"
...
cp "$TMP/render-service/docker-compose.yml" "$DIR/docker-compose.yml"
