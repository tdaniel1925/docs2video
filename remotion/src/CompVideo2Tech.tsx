...
import D from '../public/comp2/durations.json'
...
const CAPS: Record<number, { kicker?: string; head: string; accent?: string }> = {
  0: { kicker: 'The Technology Ladder', head: 'How Everyone Earns.', accent: 'Everyone' },
  1: { kicker: 'Put The Tools To Work', head: 'SmartViewz. Docs2Video.' },
  4: { kicker: 'Two Ways To Earn', head: 'Your Sales + Your Team.' },
  14: {} as any, // finale handled specially
}
const FINALE = 14 // last beat is the CTA scene
...
const PANEL_MAP: Record<string, React.FC> = { zero: P_zero, bvdef: P_bvdef, bvdef2: P_bvdef2, twoways: P_twoways, override: P_override, both: P_both, ranks: P_ranks, reqs: P_reqs, reqs2: P_reqs2, levels: P_levels, levels2: P_levels2, schedule: P_schedule, l1: P_l1, bonuses: P_bonuses, pv: P_pv, range: P_range, carriers: P_carriers, inranks: P_inranks, gens: P_gens, exteam: P_exteam, ex7500: P_ex7500, weekly: P_weekly, recruit: P_recruit, dualtree: P_dualtree, streams: P_streams }
...
const Scene: React.FC<{ i: number }> = ({ i }) => {
  ...
  return (
    <AbsoluteFill style={{ background: NAVY_D, overflow: 'hidden' }}>
      <Img src={staticFile(`comp2/f-${i}.png`)} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: `scale(${scale}) translate(${panX}%, ${panY}%)` }} />
      <Grain />
...
const IntroScreen: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', flexDirection: 'column', gap: 26, opacity: out }}>
      <Blobs />
      <div style={{ background: WHITE, borderRadius: 14, padding: '36px 60px', boxShadow: '0 26px 70px rgba(0,0,0,0.5)', transform: `translateY(${(1 - s) * 40}px) scale(${0.82 + clamp(s) * 0.18})`, opacity: clamp(s) }}><Img src={staticFile('comp2/logo.png')} style={{ height: 150, display: 'block' }} /></div>
      <div style={{ fontFamily: FONT, fontWeight: 800, fontSize: 40, color: WHITE, opacity: tagIn, transform: `translateY(${(1 - tagIn) * 12}px)`, display: 'flex', alignItems: 'center', gap: 14 }}><Star size={26} /> The Technology Ladder <Star size={26} /></div>
    </AbsoluteFill>
  )
}
const EndScreen: React.FC = () => { const f = useCurrentFrame(); const { fps } = useVideoConfig(); const s = spring({ frame: f - 4, fps, config: { damping: 14, stiffness: 140 } }); const tag = interpolate(f, [20, 40], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: EASE.expoOut }); const url = interpolate(f, [34, 54], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: EASE.expoOut }); return (<AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', flexDirection: 'column', gap: 24 }}><Blobs /><div style={{ background: WHITE, borderRadius: 14, padding: '30px 54px', boxShadow: '0 22px 60px rgba(0,0,0,0.5)', transform: `translateY(${(1 - s) * 34}px) scale(${0.86 + clamp(s) * 0.14})`, opacity: clamp(s) }}><Img src={staticFile('comp2/logo.png')} style={{ height: 120, display: 'block' }} /></div><div style={{ fontFamily: FONT, fontWeight: 900, fontSize: 50, color: WHITE, opacity: tag, textAlign: 'center' }}>Sell The Tools. <span style={{ color: '#ff8a8a' }}>Build Your Team.</span></div><div style={{ fontFamily: FONT, fontWeight: 900, fontSize: 44, color: WHITE, opacity: url }}>reachtheapex.net</div></AbsoluteFill>) }
const CornerLogo: React.FC<{ from: number; to: number }> = ({ from, to }) => { const f = useCurrentFrame(); const inO = interpolate(f, [from, from + S(0.5)], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }); const outO = interpolate(f, [to - S(0.5), to], [1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }); return <div style={{ position: 'absolute', top: 40, right: 50, opacity: Math.min(inO, outO) }}><div style={{ background: 'rgba(255,255,255,0.92)', borderRadius: 10, padding: '10px 18px', boxShadow: '0 6px 22px rgba(0,0,0,0.35)' }}><Img src={staticFile('comp2/logo.png')} style={{ height: 38, display: 'block' }} /></div></div> }

/* ---- timeline ---- */
const PAD = 0.5, XF = 0.4
const segD = vo.map((d) => (d || 6) + PAD)
const bodyFrames = Math.round((segD.reduce((a, b) => a + b, 0) - (segD.length - 1) * XF) * FPS)
export const COMP2_FRAMES = INTRO - INTRO_XF + bodyFrames + END - END_XF
...
export const CompVideo2Tech: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: NAVY_D }}>
      <Audio src={staticFile('comp2/musicDucked.mp3')} volume={1} />
      <Audio src={staticFile('comp2/voMaster.mp3')} volume={1} />
              ...
              <AbsoluteFill>
                {panel ? (
                  <AbsoluteFill style={{ background: NAVY_D }}>
                    ...
                    <Img src={staticFile(`comp2/f-${bgScene(i)}.png`)} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(0.45) saturate(1.05)', transform: 'scale(1.12)' }} />
                  ...
                  <AbsoluteFill><Scene i={i} />
                    <CaptionCard c={{ kicker: 'The Technology Ladder', head: 'Nine Ranks. Seven Levels. No Ceiling.', accent: 'No Ceiling.' }} />
                  </AbsoluteFill>
                ) : (
