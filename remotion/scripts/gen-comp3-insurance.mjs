// COMP VIDEO 3 — THE INSURANCE LADDER (deep, ~2-3 min).
// Correct to APEX_COMP_ENGINE_SPEC. Torn-paper (FLUX), Rachel VO, EL music.
// Writes public/comp3/. Reuses master-audio + CompVideo3 composition.
// REP-FACING: no waterfall / company / vendor cut. Commissions come from
// first-year-premium percentages (a standard insurance model), NOT the waterfall.
import { readFileSync, mkdirSync, existsSync, writeFileSync, copyFileSync } from 'fs'
import { execFileSync } from 'child_process'
import { join, dirname } from 'path'
import { fileURLToPath } from 'url'
const HERE = dirname(fileURLToPath(import.meta.url)); const ROOT = join(HERE, '..', '..')
const OUT = join(HERE, '..', 'public', 'comp3'); mkdirSync(OUT, { recursive: true })
...
const log = (...a) => console.log('[comp3]', ...a)
...
const BEATS = [
  ["This is the Apex Insurance ladder — for licensed life-insurance agents.", 'S'],                                            // 0
  ["Here you're paid a commission on the first-year premium of every policy you write.", 'S'],                                 // 1
  ["Your commission rate rises with your rank — from fifty percent, all the way up to ninety.", 'range'],                       // 2
  ["There are seven ranks, from Pre-Associate to M.G.A., each with a higher rate.", 'inranks'],                                 // 3
  ["You climb by production and by quality — writing consistent, well-placed business.", 'S'],                                 // 4
  ["You're contracted directly with top carriers — so your commissions are paid straight to you.", 'carriers'],                // 5
  ["And the business you write is yours. You own your book — it stays with you as you grow.", 'ownbook'],                       // 6
  ["Build a team, and you earn overrides on their production, six generations deep.", 'gens'],                                  // 7
  ["Here's an example. You recruit five agents, they each build, and your team grows to twenty.", 'exteam'],                    // 8
  ["If each writes around five thousand a month in premium, your overrides can reach about seventy-five hundred a month.", 'ex7500'], // 9
  ["Stay consistent, and a weekly production bonus rewards you on top — one to four percent.", 'weekly'],                       // 10
  ["Reach M.G.A. and build your own shop, and a quarterly recruiting bonus pays up to six thousand.", 'recruit'],              // 11
  ["That's the Insurance ladder — higher rates as you rank, a book you own, and overrides on your team.", 'S'],                // 12
]
...
const SCENE_PROMPTS = {
  0: `${PAPER} A confident red paper insurance agent figure standing beside a tall navy paper ladder, a small paper shield of protection nearby, insurance.`,
  1: `${PAPER} A red paper hand signing a navy paper policy document with a paper pen, first-year premium, a paper family protected under a paper umbrella.`,
  4: `${PAPER} A red paper figure climbing a navy paper ladder made of stacked quality gold-star paper rungs, production and quality.`,
  12: `${PAPER} A red paper figure at the top of a tall navy paper ladder holding a glowing paper book, owning your book of business, sunrise cream sky.`,
}
...
log('COMP3 ASSETS DONE ~' + timeline.toFixed(0) + 's')
