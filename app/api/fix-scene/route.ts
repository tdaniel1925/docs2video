 ...
 * the whole thing. The render service reuses the persisted plan + other scenes' VO; only
...
export async function POST(request: Request) {
  ...
  // pre-check the render service is up
  try {
  ...
  try {
    ...
    if (isAbort) return NextResponse.json({ success: true, videoId, charged, previewOnly: !!previewOnly }) // render service accepted, working async
    if (charged) await refundVideoCredits(user.id, charged, videoId)
