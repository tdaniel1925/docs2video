...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          console.error(`[generate-pptx ${videoId}] Slide ${i + 1} attempt ${attempt}/3:`, err.message?.slice(0, 100))
    ...
    // Logo buffer for PPTX generator (reuse from Gemini logo fetch above)
    const logoBuffer: Buffer | null = logoBase64 ? Buffer.from(logoBase64, 'base64') : null
