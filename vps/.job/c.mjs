...
// app/_lib/compliance.ts
var CARRIER_BLOCKLIST = [
  // ---- carriers ----
  "mutual of omaha",
  "united of omaha",
  "north american",
...
function productTokens(...sources) {
...
function nameRegexes(extraTokens = []) {
  const strip2 = new Set(CARRIER_BLOCKLIST);
  for (const n of extraTokens) if (n && n.length >= 4) strip2.add(n.toLowerCase());
  const terms = [...strip2].filter(Boolean).sort((a, b) => b.length - a.length);
  return terms.map(
    (t) => new RegExp(
      ...
      "\\b" + t.replace(/[.*+?^${}()|[\]\\&]/g, "\\$&") + "(?:\\s?(?:iul|life insurance company|life insurance|life|insurance company|insurance|company|policy|group|financial|iii|ii|iv|vi|v(?![a-z])|\\u2120|\\u00ae|\\u2122))*",
      "ig"
    )
  );
}
var ILLUSTRATION_SENSE = "column|columns|element|elements|basis|ledger|scale";
var GUARANTEE_SUBS = [
  [/\bguaranteed\s+(?=minimum|floor|rate|return|value|income|refund)/gi, ""],
  // keep the noun, drop the promise
...
function scrubComplianceText(input, extraTokens = []) {
  ...
  const danglingArticle = !!am && out !== input && out.slice(am[0].length).trimStart().length < input.slice(am[0].length).trimStart().length;
  ...
  out = out.replace(/(^|\s)\+?\s*(?:iii|ii|iv|vi)\b/gi, "$1").replace(/(^|\s)\+(?=\s|$)/g, "$1").replace(/\s{2,}/g, " ").replace(/\s+([.,!?;:])/g, "$1").replace(/([.,!?;:])\1+/g, "$1");
  if (danglingArticle) out = out.replace(article, "");
  return out.replace(/^[\s—–\-,;:]+|[\s—–\-,;:]+$/g, "").replace(/^([a-z])/, (m) => m.toUpperCase()).trim();
}
async function smoothScrubbed(pairs) {
...
function complianceLeaks(...hays) {
  const hay = hays.map((h) => typeof h === "string" ? h : JSON.stringify(h ?? "")).join(" ").toLowerCase();
  return CARRIER_BLOCKLIST.filter((t) => hay.includes(t));
}
var COMPLIANCE_CLAUSE = `
